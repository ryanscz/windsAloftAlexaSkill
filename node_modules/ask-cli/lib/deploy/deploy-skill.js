...
function deploySkill(skillId, preprocessedSkillSchema, skillInfo, profile, doDebug, callback) {
    ...
    if (!skillId || (typeof skillId === 'string' && !skillId.trim())) {
        ...
        checkClone(skillInfo, profile, () => {
            ...
            let updateSkillSpinner = new displayUtil.Spinner(' Updating skill...');
...
function updateSkill(skillId, manifest, profile, doDebug, callback) {
    // ask deploy will only call update-skill with development stage
    apiWrapper.callUpdateSkill(skillId, CONSTANTS.SKILL.STAGE.DEVELOPMENT, manifest, profile, doDebug, () => {
...
function checkSkillStatus(skillId, profile, doDebug, callback) {
    ...
    apiWrapper.callGetSkillStatus(skillId, [CONSTANTS.SKILL.RESOURCES.MANIFEST], profile, doDebug, (data) => {
        ...
        let status = jsonUtility.getPropertyFromJsonObject(buildResponse, skillStatusPathToStatus);
        ...
        apiWrapper.callGetSkillStatus(skillId, [CONSTANTS.SKILL.RESOURCES.MANIFEST], profile, doDebug, (data) => {
            ...
            if (operation.retry(isRetryFinished ? null : 'needRetry')) {
                if (data.statusCode !== 404) {
                    ...
                    let status = jsonUtility.getPropertyFromJsonObject(skillStatus, [CONSTANTS.SKILL.RESOURCES.MANIFEST, 'lastUpdateRequest', 'status']);
                    if (status !== CONSTANTS.SKILL.BUILD_STATUS.IN_PROGRESS) {
                        callback(status);
            ...
            if (operation.attempts() === CONSTANTS.CONFIGURATION.RETRY.GET_MANIFEST_STATUS.MAX_RETRY) {
                callback(CONSTANTS.SKILL.BUILD_STATUS.IN_PROGRESS);
                isRetryFinished = true;
            }
        });
    });
...
function handleSkillStatus(skillStatus, skillId, callback) {
    if (skillStatus === CONSTANTS.SKILL.BUILD_STATUS.SUCCESS) {
        console.log('Skill Id: ' + skillId);
        console.log('Skill deployment finished.');
        if (typeof callback === 'function' && callback) {
            callback(skillId);
        }
    } else if (skillStatus === CONSTANTS.SKILL.BUILD_STATUS.IN_PROGRESS) {
