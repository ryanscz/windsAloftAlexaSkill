...
function cloneISPFromSkill(skillId, skillProjectPath, profile, doDebug, callback) {
    getISPFromSkill(skillId, ispConstants.ISP_STAGE.DEVELOPMENT, profile, doDebug, (error, ispInfoList) => {
        if (error) {
            callback(error);
            return;
        }
        ...
        mkdirp.mkdirP(ispUtil.getISPRootPath(skillProjectPath), (mkdirpError) => {
            ...
            cloneISPs(skillProjectPath, ispInfoList, (error, clonedISPList) => {
                if (error) {
                    callback(error);
                    return;
                }
 ...
 * @param doDebug ASK CLI debug mode
 * @param callback the callback function that handles response (error, stage, ispList)
 */
function getISPFromSkill(skillId, stage, profile, doDebug, callback) {
    getISPListForSkill(skillId, stage, profile, doDebug, (error, inSkillProductSummaryList) => {
        ...
        let ispIdList = [];
        inSkillProductSummaryList.forEach(ispSummary => {
            ispIdList.push(ispSummary.productId);
        });

        getISPs(ispIdList, profile, doDebug, (error, ispInfoList) => {
            if (error) {
                callback(error);
                return;
            }
 ...
 * @param ispInfoList a list of objects contains ISP productId, definition and eTag
...
function cloneISPs(skillProjectPath, ispInfoList, callback) {
    ...
    let ispRootPath = ispUtil.getISPRootPath(skillProjectPath);
    let fileNameMap = new Map();

    async.doWhilst(
        (loopCallback) => {
            let ispInfo = ispInfoList[ispIndex];
            let productId = ispInfo.productId;

            let ispDefinition = ispInfo.definition;
            let ispReferenceName = ispDefinition.referenceName;
            let ispType = ispDefinition.type;
            let clonePathSub = path.join(ispRootPath, ispType.toLowerCase());
...
function downloadISPFiles(ispIdList, skillProjectPath, profile, doDebug, callback) {
    ...
    getISPs(ispIdList, profile, doDebug, (getISPsError, ispInfoList) => {
        if (getISPsError) {
            callback('Failed to download in-skill product(s): ' + getISPsError);
            return;
        }
        cloneISPs(skillProjectPath, ispInfoList, (cloneISPsError, ispInfoList) => {
            if (cloneISPsError) {
