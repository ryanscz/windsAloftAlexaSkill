...
function isISPNameValid(ispName) {
    ...
    let filteredName = ispName.match(/([a-zA-Z0-9-_]+)/g);
...
function isAtSkillRoot() {
    ...
    let skillFilePath = path.join(currentDirectory, 'skill.json');
    let configFilePath = path.join(currentDirectory, '.ask', 'config');
...
function handleAPIResponse(apiName, response, callback) {
    ...
    let responseBody = response.body ? tools.convertDataToJsonObject(response.body) : null;
...
function getISPConfigs(skillProjectPath, profile) {
    let projectConfigFile = getConfigPath(skillProjectPath);
    ...
    let propertyPathArray = ['deploy_settings', profile, 'in_skill_products'];
    let ispConfigs = jsonUtility.getProperty(projectConfigFile, propertyPathArray);
...
function getISPFiles(skillProjectPath, curDir, fileSet, depth) {
    ...
    depth++;
    let dirList = fs.readdirSync(curDir);

    dirList.forEach(dir => {
        let dirPath = path.join(curDir, dir);
