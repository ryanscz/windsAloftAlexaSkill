...
function buildUpdateSkillCommand(commander) {
    commander
        ...
        .usage('<-s|--skill-id <skill-id>> [-g|--stage <stage>] <-f|--file <file-path>> [-p|--profile <profile>] [--debug]')
    ...
    function handle(options) {
        ...
        let profile = profileHelper.runtimeProfile(options.profile);
        let manifest = jsonUtility.read(options.file);
        apiWrapper.callUpdateSkill(options.skillId, stage, manifest, profile, options.debug, () => {
